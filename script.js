document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateEloProgressBarForList(container) {
        ...
        if (!bar) return;
        const config = thresholds.elo; let percentage = 0; let barColor = 'var(--bar-color-bad)';
        if (elo !== null && !isNaN(elo) && elo > 0) {
            percentage = Math.min(100, Math.max(0, (elo / config.max) * 100));
            if (elo >= config.good) { barColor = 'var(--bar-color-good)'; } else if (elo >= config.okay) { barColor = 'var(--bar-color-okay)'; } else { barColor = 'var(--bar-color-bad)'; }
        } else { percentage = 0; barColor = 'var(--bar-background)'; }
        bar.style.width = `${percentage}%`; bar.style.backgroundColor = barColor;
    }
    ...
    function displayPlayerList(players) {
        ...
        players.forEach((player) => {
            ...
            if (player.error) {
                listItem.classList.add('error-item'); listItem.innerHTML = `<span class="player-info" style="justify-content: flex-start;">${player.nickname} - Fehler: ${player.error}</span>`;
            } else {
        ...
        statItems.forEach(item => {
            ...
            if (value !== null && value !== undefined && !isNaN(value)) {
                ...
                if (value >= config.good) {
                    barColor = 'var(--bar-color-good)'; indicatorText = 'GOOD'; valueClass = 'good';
                } else if (value >= config.okay) {
                    barColor = 'var(--bar-color-okay)'; indicatorText = 'OKAY'; valueClass = 'okay';
                } else {
                    barColor = 'var(--bar-color-bad)'; indicatorText = 'BAD'; valueClass = 'bad'; // Changed from POOR to BAD
                }
    ...
    function displayDetailCard(player) {
        ...
        if (!player || player.error) {
            cardElement.classList.add('error-card'); const errorMessage = player ? player.error : "Spielerdaten konnten nicht geladen werden."; const nickname = player ? player.nickname : "Unbekannt";
        ...
        requestAnimationFrame(() => {
            cardElement.style.opacity = '1'; cardElement.style.transform = 'translateX(0)';
            setTimeout(() => { if (player && player.nickname === currentlyDisplayedNickname) { cardElement.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } }, 100);
        });
    }
    ...
    async function loadAllPlayers() {
        loadingIndicator.style.display = 'block'; errorMessageElement.style.display = 'none'; errorMessageElement.textContent = ''; playerListContainerEl.innerHTML = '';
        hideDetailCard(); allPlayersData = []; let playerNicknames = [];
        try {
            const response = await fetch('/players.json'); if (!response.ok) { throw new Error(`Fehler Laden players.json: ${response.status}`); }
            playerNicknames = await response.json(); if (!Array.isArray(playerNicknames) || playerNicknames.length === 0) { throw new Error("players.json leer oder im falschen Format."); }
            const playerPromises = playerNicknames.map(nickname => getPlayerData(nickname)); const results = await Promise.allSettled(playerPromises);
            allPlayersData = results.map(result => { if (result.status === 'fulfilled') { return result.value; } else { console.error("Promise rejected:", result.reason); const errorMessage = result.reason?.message || 'Unbekannter Fehler'; const match = errorMessage.match(/Spieler "([^"]+)" nicht gefunden/); const nickname = match ? match[1] : 'Unbekannt'; return { nickname: nickname, error: errorMessage, sortElo: -1, level: 0 }; } });
            allPlayersData.sort((a, b) => { const aHasError = !!a.error; const bHasError = !!b.error; if (aHasError && !bHasError) return 1; if (!aHasError && bHasError) return -1; if (aHasError && bHasError) { return a.nickname.localeCompare(b.nickname); } return (b.sortElo || 0) - (a.sortElo || 0); });
    ...
    playerListContainerEl.addEventListener('click', (event) => {
        const clickedItem = event.target.closest('li'); if (clickedItem && !clickedItem.classList.contains('error-item')) { const nickname = clickedItem.dataset.nickname; if (nickname) { if (nickname === currentlyDisplayedNickname) { hideDetailCard(); } else { const playerData = allPlayersData.find(p => p.nickname === nickname); if (playerData) { displayDetailCard(playerData); } else { console.error("Daten nicht gefunden für:", nickname); hideDetailCard(); } } } }
    });
    ...
    if (refreshButton) { refreshButton.addEventListener('click', () => { console.log("Daten werden manuell neu geladen..."); loadAllPlayers(); }); }
});